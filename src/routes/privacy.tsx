...
export const Route = createFileRoute("/privacy")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "SULTAN Privacy Policy for Pi Network authentication and ethical data stewardship." },
    ...
    <main className="sultan-bg relative min-h-screen overflow-hidden text-white">
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-4xl flex-col px-4 py-8 sm:px-6 sm:py-14">
        <header className="mb-8 border-b border-amber-200/15 pb-8">
          <nav aria-label="Legal navigation" className="mb-10 flex flex-wrap items-center justify-between gap-4 text-sm">
            ...
            <div className="flex gap-4 text-white/60">
              <Link to="/privacy" className="text-amber-200" aria-current="page">Privacy</Link>
              <Link to="/terms" className="transition-colors hover:text-amber-200">Terms</Link>
            </div>
          </nav>
          <p className="mb-3 text-right text-sm text-amber-200/75" dir="rtl">سياسة الخصوصية</p>
          <h1 className="sultan-gold-text text-3xl font-bold tracking-tight sm:text-5xl">Privacy Policy</h1>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-white/65">A clear statement of how SULTAN protects trust, limits collection, and handles Pi Network account information.</p>
        ...
        <div className="sultan-glass mb-8 rounded-2xl p-5 sm:p-6">
          <p className="text-sm leading-7 text-amber-100/85">SULTAN follows principles of trust, honesty, justice, and responsible technology. This policy applies to the SULTAN application and its Pi Browser integration.</p>
        </div>
        ...
        <footer className="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-amber-200/15 pt-6 text-sm text-white/55">
          ...
          <Link to="/terms" className="text-amber-200 transition-colors hover:text-white">Read Terms of Service →</Link>
        </footer>
      </div>
    </main>
  );
